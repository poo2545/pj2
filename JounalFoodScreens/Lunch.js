...
import { View, Text, TouchableOpacity, StyleSheet, TextInput, SafeAreaView, FlatList, Modal, Image , Button } from 'react-native';
...
const [LCarbohydrate, setLCarbohydrate] = useState('');
...
const [isCalendarVisible, setCalendarVisible] = useState(false);
  ...
  const saveMeal = async () => {
    try {
      const existingMeals = await axios.get(`http://${apiBaseUrl}:8000/meals?userId=${userId}&date=${date}`);
  
      if (existingMeals.data.length > 0) {
        console.log('Meal already exists:', existingMeals.data);
      } else {
